...
ggplot(reg_dfw, 
  ...
  geom_line(data = nd, 
            ...
            color = "forestgreen") +
  geom_ribbon(data = nd, aes(y = mod1_yield_mgha, ymin = mod1_int_lcl, ymax = mod1_int_upl), fill = "gray", alpha =0.5)
```
...
# Model fitting
mod3_quad <- lmer(data = reg_dfw, yield_mgha ~ (1 |rep) + sr_ksha + I(sr_ksha^2))
...
# Model fitting
mod4_linp <- nlme(data = reg_dfw, yield_mgha ~ SSlinp(sr_ksha, a, b, xs), 
...
ggplot(reg_dfw, aes(x = sr_ksha, y = yield_mgha))+
  ...
  geom_line(data=nd,aes(y= mod1_yield_mgha),
             color = "forestgreen",
             size = 1.5)+
   geom_line(data=nd,aes(y= mod2_yield_mgha),
             color = "blue",
             size = 1.5)+
   geom_line(data=nd,aes(y= mod3_yield_mgha),
             color = "purple",
             size = 1.5)+
...
predict(mod4_linp, newdata = data.frame(sr_ksha = 73.48), level = 0)
...
ggplot(reg_dfw, aes(x = sr_ksha, y = yield_mgha))+
  ...
  geom_line(data = nd, aes(y = mod4_yield_mgha), 
            color = "orange",
            size = 1.5) +
  geom_segment(x =73.48, xend = 73.45, y = 13.45, yend = 0)+
  geom_segment(x = 73.48, xend = 0, y =13.45, yend = 13.45)
...
library(infer)
#we need to add something heere
```
